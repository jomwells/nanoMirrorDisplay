...
int powerbutton = 7;         // output for power button D7
int sidepirsensor = 2;       // input (digital) for PIR pin D2
int frontpirsensor = 3;      // input (digital) for PIR pin D3
int displaysensor = A0;      // input (Analog) for display Sensor A0
int displaydigitalout = 8;   // output (digital) for display sensor D8
...
void setup() {
  ...
  if (displayval > 450){ // if the display is on
    ...
    if (sidepirval == HIGH){ // if the display is on & motion is detected ****!!! change to front !!!****
    ...
    if (sidepirval == LOW){ // if the display is on & motion is not detected ****!!! change to front !!!****
      Serial.println("Display is already ON");
